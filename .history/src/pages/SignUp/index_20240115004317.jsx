...
function index() {
    ...
    const onSubmit = async (event) => {
        ...
        try {
            ...
            const _error = responseError.response?.data;
            if (_error && _error.status === 400) {
                setErrors(_error.validationErrors)
                for (let i = 0; 0 < Object.keys(_error.validationErrors).length; i++) {
                    openNotification("Hata Oluştu !", _error.validationErrors[Object.keys(_error.validationErrors)[i]], "warning", 'topRight');
                }
            }
            else {
