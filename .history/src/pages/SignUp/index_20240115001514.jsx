...
function index() {
    ...
    const onSubmit = async (event) => {
        ...
        try {
            ...
            if (responseError.response?.data && responseError.response?.data.status === 400) {
                ...
                openNotification("Hata Oluştu !", responseError.response?.data.validationErrors, "warning", 'topRight');
            }
            else {
    ...
    return (
        <Form
            ...
            <FormItem
                ...
                name="username"
                autoComplete='false'
                // message={!_.isEmpty(errors) ? errors.username : ""}
                message={errors.map(e =>e.username)}
            ...
            <FormItem
                ...
                name="email"
                autoComplete='false'
                // message={!_.isEmpty(errors) ? errors.email : ""}
                message={errors.map(e =>e.email)}
            ...
            <FormItem
                ...
                isPassword={true}
                autoComplete='false'
                // message={!_.isEmpty(errors) ? errors.password : ''}
                message={errors.map(e =>e.password)}
    ...
    function customNotification() {
        return (message, description, type, placement) => {
            api[type]({
                message: message,
