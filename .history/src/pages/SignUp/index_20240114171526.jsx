...
function index() {
    ...
    const [api, contextHolder] = notification.useNotification();
     ...
     const customNotification = customNotification(message, description, type, placement);
    ...
    const onSubmit = async (event) => {
        ...
        try {
            ...
            customNotification("Yeni Kullanıcı", response.data.message, "success", 'topRight');
        } catch (responseError) {
            if (responseError.response?.data && responseError.response?.data.status === 400) {
                setErrors(responseError.response?.data.validationErrors)
                customNotification("Hata Oluştu !", responseError.response?.data.validationErrors, "warning", 'topRight');
            }
            else {
                customNotification("Hata Oluştu !", "Tahmin Edilemeyen Hatalarda gösterilecektir. / Unexpected error occured. Pleace try again.", "error", 'topRight');
            }
        } finally {
