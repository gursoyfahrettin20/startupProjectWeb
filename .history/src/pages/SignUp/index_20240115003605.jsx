...
function index() {
    ...
    const onSubmit = async (event) => {
        ...
        try {
            ...
            const _error = responseError.response?.data.validationErrors;
            if (responseError.response?.data && responseError.response?.data.status === 400) {
                setErrors(_error)
                for (let i = 0; 0 < Object.keys(_error).length; i++) {
                    openNotification("Hata Oluştu !", Object.keys(_error)[i], "warning", 'topRight');
                }
            }
            else {
